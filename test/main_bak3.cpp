...
//ofn_lpstrFile =  fdisk;
    ...
    try {
		...
		_bstr_t strConn = "Provider='vfpoledb.1'; Data Source='" + strFolderName + "'; SourceType=DBF;";
		...
		PrintComError(e);
		try {
			_bstr_t strConn = "Provider=MSDASQL.1;Driver={Microsoft Visual FoxPro Driver};UID=;PWD=;SourceDB=" + strFolderName + ";SourceType=DBF;Exclusive=Yes;BackgroundFetch=No;Collate=Machine;";
	...
	//_bstr_t m_cRec = "ALTER TABLE " + strFileName + " ADD COLUMN FOR_POM7 N(0,0)";
	_bstr_t m_cRec = "";
	...
	try {
		...
		m_cRec = "UPDATE " + strFileName + "  SET FOR_POM=3"; cnn->Execute(m_cRec,NULL,1);

		m_cRec = "UPDATE " + strFileName + "  SET AIM=5"; cnn->Execute(m_cRec,NULL,1);
...
//
//BOOL InitInputFile(int nCmdShow)
